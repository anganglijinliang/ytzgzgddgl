...
1.  **用户认证与权限管理**
    -   支持多角色登录：管理员 (admin)、录入员 (entry)、生产人员 (prod)
    -   基于角色的菜单和功能权限控制
...
3.  **生产跟踪 (Workshop Mode)**
    -   **车间触屏模式**：专为车间平板/工控机设计的大按钮交互界面 (NumPad)
    -   **环境灵活配置**：支持随时切换生产日期、所属车间、班组（甲乙丙丁）及班次（白中夜），支持历史数据补录
    -   **容错保护**：内置 ErrorBoundary 防止白屏，关键操作二次确认
    -   **实时反馈**：全屏成功提示动画，今日产量实时统计
    -   **质量参数录入 (Quality 4.0 Lite)**：支持水压 (压力/时间)、内衬 (厚度)、外防 (锌层重量) 等关键工艺参数录入
    -   **按班组（甲乙丙丁）、班次（白中夜）记录生产**
    -   **自动归档**：生产记录自动关联到所选日期的报表
...
4.  **企业级特性**
    ...
    -   **报表查询**：多维度筛选与数据可视化仪表盘
    -   **字体优化**：针对中文环境优化（Microsoft YaHei, PingFang SC），无需下载外部字体

5.  **响应式设计**
    -   **移动端优先**：订单列表在手机端自动转换为卡片视图，操作更便捷
    -   **完美适配**：兼容移动端 (Mobile)、平板 (Tablet) 和桌面端 (Desktop)
